...
@pytest.fixture
def repository() -> InMemoryPersonRepository:
    return InMemoryPersonRepository()


def test_add_person(repository: InMemoryPersonRepository) -> None:
    person = Person(name="John Doe", age=30, id=1)
    repository.add(person)

    retrieved_person: Person | None = repository.get_by_id(1)
    assert retrieved_person is not None
    assert retrieved_person.name == person.name
    assert retrieved_person.age == person.age


def test_update_person(repository: InMemoryPersonRepository) -> None:
    person = Person(name="John Doe", age=30, id=1)
    repository.add(person)
    ...
    updated_person: Person | None = repository.get_by_id(1)
    assert updated_person is not None
    assert updated_person.name == "Jane Doe"
    assert updated_person.age == 28


def test_delete_person(repository: InMemoryPersonRepository) -> None:
    person = Person(name="John Doe", age=30, id=1)
    repository.add(person)
    repository.delete(1)

    deleted_person: Person | None = repository.get_by_id(1)
