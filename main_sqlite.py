...
@contextmanager
def create_database_connection() -> Generator[sqlite3.Connection, None, None]:
    db_connection: sqlite3.Connection = sqlite3.connect("./db/data.db")
    try:
        yield db_connection
    finally:
        db_connection.close()
...
with create_database_connection() as conn:
    ...
    new_order = Order(
        person_id=None, order_date="2023-04-03", total_amount=100.0
    )
